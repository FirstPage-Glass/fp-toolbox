...
- `NavBar` renders a minimal placeholder until mount (`mounted === false`) to avoid hydration mismatch, then fetches `/api/me` for auth state. Preserve this pattern — no client-side cookie reads.
- Auth flow: `/login` → `POST /api/login` → shared httpOnly `fp_session` (domain=`FP_SESSION_DOMAIN`, ≤8h) — the identity when `FP_MCP_INTERNAL_KEY` is set — plus host-only `fp-auth` (legacy mode / usage attribution). Identity is validated on every request (proxy + routes) via `getSessionUser()`/`currentUsername()` in `lib/auth.ts` against mcp `GET /admin/api/session` (SSO) or `AUTH_USERS` (legacy); fetch errors degrade to unauthenticated. `proxy.ts` redirects unauthenticated users to `/toolbox`; logout clears both cookies. `/toolbox`, `/login`, `/api/*` stay public.

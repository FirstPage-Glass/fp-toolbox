"use client";

import Link from "next/link";
import { useRouter, usePathname } from "next/navigation";
import { useState, useEffect } from "react";

function getAuthCookie() {
  if (typeof document === "undefined") return null;
  return document.cookie
    .split("; ")
    .find((row) => row.startsWith("fp-auth="))
    ?.split("=")[1];
}

export default function NavBar() {
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [mounted, setMounted] = useState(false);
  const router = useRouter();
  const pathname = usePathname();

  useEffect(() => {
    setMounted(true);
    setIsLoggedIn(getAuthCookie() === "authenticated");
  }, [pathname]);

  const handleLogout = async () => {
    await fetch("/api/logout", { method: "POST" });
    document.cookie = "fp-auth=; max-age=0; path=/";
    setIsLoggedIn(false);
    router.push("/toolbox");
    router.refresh();
  };

  // Prevent hydration mismatch — render minimal nav on server
  if (!mounted) {
    return (
      <nav className="hidden md:flex items-center gap-6">
        <span className="text-sm font-medium text-slate-600">Toolbox</span>
      </nav>
    );
  }

  if (!isLoggedIn) {
    return (
      <nav className="hidden md:flex items-center gap-6">
        <Link
          href="/toolbox"
          className={`text-sm font-medium transition-colors ${
            pathname === "/toolbox"
              ? "text-fp-500"
              : "text-slate-600 hover:text-fp-500"
          }`}
        >
          Toolbox
        </Link>
        <Link
          href="/login"
          className="text-sm font-medium px-4 py-2 bg-fp-500 hover:bg-fp-600 text-white rounded-lg transition-colors"
        >
          Login
        </Link>
      </nav>
    );
  }

  return (
    <nav className="hidden md:flex items-center gap-6">
      <Link
        href="/"
        className={`text-sm font-medium transition-colors ${
          pathname === "/"
            ? "text-fp-500"
            : "text-slate-600 hover:text-fp-500"
        }`}
      >
        Overview
      </Link>
      <Link
        href="/toolbox"
        className={`text-sm font-medium transition-colors ${
          pathname === "/toolbox"
            ? "text-fp-500"
            : "text-slate-600 hover:text-fp-500"
        }`}
      >
        Toolbox
      </Link>
      <Link
        href="/systems"
        className={`text-sm font-medium transition-colors ${
          pathname === "/systems"
            ? "text-fp-500"
            : "text-slate-600 hover:text-fp-500"
        }`}
      >
        Our Systems
      </Link>
      <Link
        href="/architecture"
        className={`text-sm font-medium transition-colors ${
          pathname === "/architecture"
            ? "text-fp-500"
            : "text-slate-600 hover:text-fp-500"
        }`}
      >
        Stack
      </Link>
      <button
        onClick={handleLogout}
        className="text-sm font-medium text-slate-500 hover:text-red-500 transition-colors"
      >
        Logout
      </button>
    </nav>
  );
}

...
- `lib/mcp.ts` — firstpage MCP JSON-RPC client (`FP_MCP_API_KEY`, server-side only): `mcpCall` + typed helpers `getMcpPsi` / `getMcpGsc` (row_limit 1000 default — site-wide totals, not top-25) / `getMcpGa4` / `getMcpInventory`. GA4 `property_id` is the bare numeric id; GSC needs YYYY-MM-DD.

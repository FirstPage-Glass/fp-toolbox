...
- `lib/gateway/` — DeepSeek team-key gateway (OpenRouter BYOK hybrid: OpenRouter enforces each key's monthly USD limit, fp-toolbox manages teams/champions/members/alerting). **Multi-key model** (feat/gateway-multi-key):
  - `db.ts` — `deepseek_teams` (name/champion/**credit_usd** monthly pool + **max_keys** count, admin-controlled), `deepseek_keys` (one per OpenRouter key: hash/label/limit_usd/status/created_by), `deepseek_key_members` (key↔username, 1–2 per key, UNIQUE(key_id, username)), `deepseek_usage_snapshots` (hourly per-key spend), `deepseek_alerts_log` (dedupe per key/level/month via unique index `idx_ds_alerts_dedupe` on `(key_id, level, date_trunc('month', sent_at AT TIME ZONE 'UTC'))` — expression index needs explicit timezone to be IMMUTABLE); CREATE TABLE IF NOT EXISTS + idempotent ALTERs, same pattern as `lib/usage.ts`. Tables also mirrored in `db/init.sql`.
  - `openrouter.ts` — Management API client (`OPENROUTER_MANAGEMENT_KEY`): `createKey` (POST + PATCH `include_byok_in_limit: true`, `limit_reset: "monthly"` — BYOK spend counts against the key limit), `listKeys` (paginated; `byok_usage_monthly` is the spend source), `getKey`, `updateKey`, `deleteKey`. Plaintext key returned at body level (`{data, key}`) once, never stored.
  - `service.ts` — role resolution (`resolveRole`: admin / champion / member / none), `getTeamsView` (role-scoped), `issueKey` (validates max_keys + sum of active key limits ≤ credit_usd + members ≤2 & one active key per user), `assignMember`/`removeMember`, `revokeKey`, `updateTeamLimits` (admin-only, cannot go below allocated credit/keys).
  - `alert-scheduler.ts` — hourly poll (`GATEWAY_POLL_MINUTES`, default 60) started by root `instrumentation.ts`: snapshot per-key usage, push 80%/100% alerts to `SLACK_WEBHOOK_URL` + `deepseek_alerts_log` (in-app), deduped per key/level/month.
